...
```{r}
state_dex <- read.csv("data/state_dex_cleaned.csv")
glimpse(state_dex)
```
...
nrow(state_dex)
```
4590, each row is an individual neighborhood.
...
Potentially over-represent higher income people, and people who were more active in the pandemic. I think because of this the data might overestimate exposure. 
...
TX_dex <- state_dex %>% 
  filter(state == "TX") %>% 
    select(state, date, dex, dex_income_1, dex_income_2, dex_income_3, dex_income_4)
...
Exppsure seems to be much higher in Febuary than in April. 
...
It's created as a column, and added to the original dataframe
...
state_dex %>%
  ...
  geom_histogram() # make a histogram
```
Dex dramatically decreases in March and April.
I don't think histograms are very good for changes over time, as there cannot be a time axis.
